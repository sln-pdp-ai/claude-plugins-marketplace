...
| `pdp-po-workbench` | PO du domaine PDP, sur son dépôt de savoir | skills `policy`, `jira-story-writer`, `jira-study-writer`, `study-to-story`, `deliverable-linter`, `confluence-page-writer`, `idea-incubator`, `idea-refiner`, `project-onboarder`, `knowledge-sync` + sous-agent `codebase-syncer` + `bin/` + hooks |
...
Les deux premiers existent. Le troisième reste à écrire, et son `bin/` copiera celui du workbench.
...
Trois règles tenues, à ne pas défaire :

- La mécanique de checkout et de synchronisation de `pdp-po-workbench` est un `bin/` **autonome**
  ([bin/pdp-lib.ps1](../plugins/pdp-po-workbench/bin/pdp-lib.ps1) et `pdp-repos.ps1`), sur le modèle de
  `bin/smt-repos.ps1`, jamais de la prose diluée dans un `SKILL.md` de rédaction. Le jour où
...
- **L'état de synchronisation, lui, ne vit pas dans le plugin.** Le manifeste est statique et voyage donc
  très bien ; le SHA du dernier commit ingéré par checkout est muté à chaque run. Un plugin installé est
  une copie en cache, écrasée à chaque mise à jour : un état écrit là serait perdu sans bruit, et le run
  suivant croirait repartir de zéro. Il vit chez l'utilisateur, dans `knowledge/core/sync-state.md`, dans
  le même commit que le savoir qu'il décrit. Le dépôt source, `archi-user-stories-assistant`, mélangeait
  les deux dans un seul `projects-manifest.md` ; ce mélange ne survit pas au passage en plugin.

Écart avec le dépôt source à connaître avant de comparer les deux : son manifeste pointait des checkouts
frères en `../<dépôt>/` **sans URL git**, ce qui suffit quand les dépôts sont déjà là et ne suffit pas à
un plugin qui doit pouvoir cloner. Les URLs ont été relevées sur les checkouts réels, pas déduites d'un
motif.

`pdp-architecture` n'a ni script, ni hook, ni serveur MCP, donc il fonctionne aussi sur macOS et Linux.
`pdp-po-workbench` a un `bin/` PowerShell 5.1, donc il est Windows-only, comme les plugins `smt-*`.

### Ce que `pdp-po-workbench` bloque

Trois hooks `PreToolUse`, pour trois raisons distinctes :

| Mode | Bloque | Pourquoi |
|---|---|---|
| `atlassian` | outils MCP d'écriture Jira et Confluence | l'arbitrage retenu est « Markdown à coller » : un livrable passe par un fichier relu et versionné. Échappatoire `PDP_ALLOW_ATLASSIAN_WRITES=1` |
| `files` | écritures de fichier dans un checkout résolu | ce sont les dépôts des équipes de dev, lus en source |
| `git` | commandes git mutantes visant un checkout | sans ce mode, le précédent ne servirait à rien : un `git reset` fait les mêmes dégâts qu'un `Write` |

Le mode `git` est le plus délicat : `fetch`, `log`, `show`, `diff` et `rev-parse` doivent rester
autorisés, la synchro en dépend. Les verbes mutants sont donc ancrés juste après `git` (avec `-C <path>`
et `-c <cfg>` tolérés entre les deux), et `prune` comme `gc` sont volontairement hors de la liste :
`git fetch --prune` les ferait matcher à tort et casserait la synchro. Les six cas de cette frontière ont
été vérifiés à l'exécution.

Ce qui n'est **pas** bloqué, volontairement : les écritures dans le projet courant de l'utilisateur. Le PO
y écrit ses livrables et son savoir.
